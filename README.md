...
![image](https://github.com/user-attachments/assets/07cd7e74-4e61-4e9e-85f8-5413e5c9d196)

**总结**：  
设置零基线避免误解（如某个数据看起来是另外的两倍）。操作路径：右键坐标轴 -> 设置坐标轴格式 -> 边界的最小值为0（可同时设置最大值和单位值）。条形图宽度建议间距的1-2倍，通过双击条形图修改间隙宽度（≤100%）。

## 1.2 绘制表格数据的柱形图，并设置不同的直条间距  
![image](https://github.com/user-attachments/assets/3a1f6a78-0659-46dc-bfb1-bf8b08bf26c3)

**总结**：  
调整条形图间距：双击条形图 -> 修改间隙宽度（建议1-2倍间距），使视觉焦点集中在数据而非空白区域。

## 1.3 绘制表格数据的柱形图，并制作不同三维效果的柱形图  
![image](https://github.com/user-attachments/assets/85c1df76-22cf-40b2-9d20-f29739bb2683)

**总结**：  
三维柱形图类型包括：  
- 簇状柱形图：比较各类别数值大小  
- 堆积柱形图：显示单个项目与整体的关系  
- 百分比堆积柱形图：比较各类别数值占总值的百分比  
